...
// The tileset id portion after the username must be <= 32 chars (Mapbox limit).
const TILESET_ID = 'sustainasoft.epa-food-processors-2026-06';
const SOURCE_ID = 'epa_food_processors';
const SOURCE_NAME = `mapbox://tileset-source/sustainasoft/${SOURCE_ID}`;
const BASE_URL = 'https://api.mapbox.com';
const GEOJSON_LD_PATH = path.join(process.cwd(), 'src/data/epa-food-processor-facilities.geojson.ld');
...
async function uploadTilesetSource(token: string): Promise<void> {
  ...
  formData.append('file', new Blob([fileData], { type: 'application/x-ndjson' }), 'epa-food-processors.geojson.ld');

  // CRITICAL: use PUT, not POST. Mapbox's POST /sources APPENDS the file to any
  // existing source, so repeated runs accumulate stale features. PUT replaces the
  // source outright (same lesson as the CARB pipeline).
...
async function createOrUpdateTileset(token: string): Promise<void> {
  ...
  const createRes = await fetch(apiUrl(`/tilesets/v1/${TILESET_ID}`, token), {
    ...
    body: JSON.stringify({ recipe, name: 'EPA Food Processors Cal BioScape 2026-06' }),
  });
...
async function pollUntilComplete(token: string, jobId: string): Promise<void> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (res.status === 403) {
      // Some tokens carry tilesets:write but not tilesets:read, so the status
      // endpoint 403s. The publish was already submitted in Step 3; don't fail.
...
async function main(): Promise<void> {
  ...
  console.log(`\nPublishing EPA food processors tileset to sustainasoft Mapbox account`);
  ...
  if (!fs.existsSync(GEOJSON_LD_PATH)) {
    ...
    console.error('Run "npm run build-epa-data" first.');
    process.exit(1);
  }
